...
final getFavoritedPlaylistsUsecaseProvider = Provider<GetFavoritedPlaylistsUsecase>((ref) {
  return GetFavoritedPlaylistsUsecase(
    playlistRepository: ref.read(playlistRepositoryProvider),
  );
});

class GetFavoritedPlaylistsUsecase
    implements UsecaseWithoutParms<List<PlaylistEntity>> {
  final IPlaylistRepository _playlistRepository;

  GetFavoritedPlaylistsUsecase({required IPlaylistRepository playlistRepository})
      : _playlistRepository = playlistRepository;

  @override
  Future<Either<Failure, List<PlaylistEntity>>> call() {
    return _playlistRepository.getFavoritedPlaylists();
  }
}